    ...
    private class ConnectThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                connectionFailed(com.payment.sujan.madmoney.Connectors.Constants.SENDING);
                return;
            }
...
//            socket.close();
    ...
    private void transNotification(BluetoothDevice device, int transType) {
        Message msg;
        if (transType == com.payment.sujan.madmoney.Connectors.Constants.TRANS_TYPE_MONEY) {
            msg = handler.obtainMessage(com.payment.sujan.madmoney.Connectors.Constants.MESSAGE_MONEY_SENT);
        } else {
            msg = handler.obtainMessage(com.payment.sujan.madmoney.Connectors.Constants.MESSAGE_MESSAGE_SENT);
        }
        Bundle bundle = new Bundle();
        bundle.putString(com.payment.sujan.madmoney.Connectors.Constants.DEVICE_NAME, device.getName());
        msg.setData(bundle);
    ...
    private void connectionFailed(String type) {
        ...
        Message msg = handler.obtainMessage(com.payment.sujan.madmoney.Connectors.Constants.CONNECTION_FAILED);

        Bundle bundle = new Bundle();

        bundle.putString(com.payment.sujan.madmoney.Connectors.Constants.CONN_FAIL_WHILE, type);
